...
import { getMenuData } from '@/api/routers'
import Main from '_c/main' // Main 是架构组件，不在后台返回，在文件里单独引入
import parentView from '_c/parent-view' // parentView 是二级架构组件，不在后台返回，在文件里单独引入
...
export const initRouter = (vm) => {
  ...
  var routerData
  if (!gotRouter) {
    getMenuData(store.state.user).then(res => {
